...
function showusers($kolpo){
  global $users,$q; 
    ...
    echo '<tr><td align="right" valign="top">
          ...
          <th><input id="p'.$q.'" type="password"  name="password'.$q.'"  value="'.$users[$q]['password'].'" required></th> 
          <th><select id="s'.$q.'" name="rolos'.$q.'" style="color: blue;" '.$kolpo.' >
          <option value="admin" '.$admin.'>Admin</option>
          <option value="author" '.$author.'>Author</option>
          </select></th> 
          <th><input type="text" name="name'.$q.'" value="'.$users[$q]['name'].'" required></th>
...
<?php
  for ($q=1;$q<=$make_users;$q++){
     ...
     if ($_SESSION['UserData']['rolos']=="author" && $users[$q]['username']==$_SESSION['UserData']['Username'] ){
        showusers('disabled');
       }
    } 
     ?>
